...
//	GeneralRun *r = new GeneralRun();
	...
	{
		MessageBox(0,_T("To install Codex.net R4 Application you must be log as an Administrator"), _T("Codex.net R4 Installer"), MB_OK | MB_ICONINFORMATION);
    ...
    {
		MessageBox(NULL, _T("To Install Codex DS, You need at least Windows 7 with SP1"), _T("Windows Version Not Supported"), MB_ICONERROR | MB_OK);
		// Delete the shell manager created above.
		if (pShellManager != NULL)
		{
			delete pShellManager;
	...
	if (ili9->IsNetfx461OrAboveInstalled() == false)
	{
		if (MessageBox(NULL, _T("To run Codex DS setup you should have at least .NET Framework 4.6.1 in your computer\n Do you want to install .net framework"), _T(" Warning "), MB_ICONWARNING | MB_YESNO | MB_DEFBUTTON2) == IDNO )
		{
...
/*
		...
		MessageBox(0, _T("Error launching Codex DS 1.5 Installer"), _T("Codex.net DS 1.5 Installer"), MB_OK | MB_ICONINFORMATION);
