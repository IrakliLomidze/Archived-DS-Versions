	...
	public class Database
	{
		...
		public int ProcessFiles()
		{
			...
			{ 
                ...
                Owner.ShowProgress(0, "ფაილის გახსნა");
                Owner.ShowProgress(0, "------------------------------------------------------------------------");
				return 10;
			}
		...
		#region update Database
			...
			*/
			...
			if (File.Exists(Path.GetDirectoryName(Form1.CopyTo)+@"\databasePS.info") == true)
			{
                if ((File.GetAttributes(Path.GetDirectoryName(Form1.CopyTo) + @"\databasePS.info") & FileAttributes.ReadOnly) == FileAttributes.ReadOnly)
                { File.SetAttributes(Path.GetDirectoryName(Form1.CopyTo) + @"\databasePS.info", System.IO.FileAttributes.Archive); }
			}

			
			
			File.Copy(Path.GetDirectoryName(Form1.CopyFrom)+@"\databasePS.info",Path.GetDirectoryName(Form1.CopyTo)+@"\databasePS.info",true);
...
//			System.Windows.Forms.MessageBox.Show(srv.Databases.Item(1,"dbo").Name);
			...
			if (res1 == true)
			{
				try
				{
					Owner.ShowProgress(0,"ძველი CodexPS ბაზის წაშლა");
                    srv.DetachDatabase("CodexPS",true,true);
					Owner.ShowProgress(0,"წაშლილია");
				}
				catch 
				{
					Owner.ShowProgress(0,"Error: ");
                    Owner.ShowProgress(0, "არ ხერხდება CodexPS ბაზის გადაწერა, ბაზაზე მუშობს მომხმარებელი ");
...
//			StringBuilder strx = new StringBuilder();
			...
			try
			{
                srv.AttachDatabase("CodexPS", strx);// AttachDB("Codex2005", strx.ToString());
				Owner.ShowProgress(0,"CodexPS რეგისტრირებულია");
			}
			catch (Exception e)
			{
				Owner.ShowProgress(0,"Error: ");
				Owner.ShowProgress(0,"არ ხერხდება CodexPS ბაზის რეგისტრაცია ");
