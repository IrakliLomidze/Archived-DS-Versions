...
//            this.textControl1.SelectAll();
  //          this.textControl1.Selection.Text = "";
            ...
            this.textControl1.PageSize = new Size(ILG.Codex.Codex2007.Properties.Settings.Default.DocumentPageWidth,
                                                  ILG.Codex.Codex2007.Properties.Settings.Default.DocumentPageHeight);
            //this.textControl1.PageSize.Width = ILG.Codex.Codex2007.Properties.Settings.Default.DocumentPageWidth;
        ...
        public int EditDocumentMode(int ID)
        {
            ...
            string strsql = "Select " + Fields + " FROM CodexPS_DDOCS WHERE C_ID = " + ID.ToString() + " ";
           ...
           // CLinkGrid.InitializeVarialbles(2);
            ...
            if ((HasAttachments == true) && (License.IsAttachmentShow() == true))
            {
                ...
                if (zipfile.GetFiles(false).Length != 0)
                {
                    ...
                    //for (int ii = 0; ii < zf3.Entries.Count; ii++)
                    {
        ...
        private void SaveDocument()
        {
			...
			SaveSettings.PageMargins = this.textControl1.PageMargins;
            SaveSettings.PageSize = textControl1.PageSize;
...

			{
                ...
                if (str == ".RTF") { this.textControl1.Save(fd.FileName, TXTextControl.StreamType.RichTextFormat); return; }
                if (str == ".DOC") { this.textControl1.Save(fd.FileName, TXTextControl.StreamType.MSWord); return; }
                if (str == ".DOCX") { this.textControl1.Save(fd.FileName, TXTextControl.StreamType.WordprocessingML); return; }
        ...
        private int detectzoom(int c)
        {
            ...
            int nPageWidthInPixels = (textControl1.PageSize.Width
                // - F_CGL_DOC.textControl_CGL.PageSize.PageMargins.Left - F_CGL_DOC.textControl_CGL.PageMargins.Right
                                       );
            int nPageHeightInPixels = (textControl1.PageSize.Height
        ...
        private void DocumentAddEdit_Load(object sender, EventArgs e)
        {
            ...
            this.Height = System.Windows.Forms.Screen.PrimaryScreen.WorkingArea.Height - this.CodexDocumentStatusBar.Height; 
        ...
        private void ultraToolbarsManager1_BeforeRibbonTabSelected(object sender, Infragistics.Win.UltraWinToolbars.BeforeRibbonTabSelectedEventArgs e)
        {
                ...
                {
                    case "RText": this.ultraTabControl1.SelectedTab = this.ultraTabControl1.Tabs[0]; textControl1.Focus();  break;
                    case "RAttribute": this.ultraTabControl1.SelectedTab = this.ultraTabControl1.Tabs[1]; break;
        ...
        public void LoadTables()
        {
            try
            {
                SqlDataAdapter da = new SqlDataAdapter(
                                       "SELECT * FROM CodexPS_DAUTHOR ORDER By A_Order;" +
                                       "SELECT * FROM CodexPS_DTYPE ORDER By T_Order;" +
                                       "SELECT * FROM CodexPS_DSubject ORDER By S_Order;" +
                                       "SELECT * FROM CodexPS_DWords ORDER By W_Order;" +
                                       "SELECT * FROM CodexPS_DCategory ORDER By C_Order;" +
                                       "SELECT * FROM CodexPS_DStatus ORDER By C_Order;",
                                       global::ILG.Codex.Codex2007.Properties.Settings.Default.ConnectionString);
                DS = new DataSet();
                DataTableMapping dtm3, dtm4, dtm5, dtm6,dtm7, dtm8;
                dtm3 = da.TableMappings.Add("Table", "CodexPS_DAUTHOR");
                dtm4 = da.TableMappings.Add("Table1", "CodexPS_DTYPE");
                dtm5 = da.TableMappings.Add("Table2", "CodexPS_DSubject");
                dtm6 = da.TableMappings.Add("Table3", "CodexPS_DWords");
                dtm7 = da.TableMappings.Add("Table4", "CodexPS_DCategory");
                dtm8 = da.TableMappings.Add("Table5", "CodexPS_DStatus");
                da.Fill(DS);
                // Visited
        ...
        {
            CAuthor.DataSource = DS.Tables["CodexPS_DAUTHOR"];
            ...
            CType.DataSource = DS.Tables["CodexPS_DTYPE"];
            ...
            CCategory.DataSource = DS.Tables["CodexPS_DCategory"];
 ...
 //           CCategory.DisplayLayout.Grid.Width = CAuthor.Width;
            ...
            //CSbject
            CSbject.DataSource = DS.Tables["CodexPS_DSubject"];
            ...
            //CWords
            CWords.DataSource = DS.Tables["CodexPS_DWords"];
            ...
            //CStatus
            CStatus.DataSource = DS.Tables["CodexPS_DStatus"];
        ...
        private void ultraButton7_Click(object sender, EventArgs e)
        {
            ...
            Point dc = ultraTabPageControl12.PointToScreen(dc2);
            dlg2.Location = dc;
            dlg2.ShowDialog();
            ...
            if (fdd.ShowDialog() == DialogResult.OK)
            {
                ...
                if (zipfile.GetFiles(false).Length > 0)
                {
                    ...
                    foreach (AbstractFile f in zipfile.GetFiles(false))
                    {
                        ...
                        if (Path.GetFileName(f.FullName.ToString()).ToUpper().Trim() == System.IO.Path.GetFileName(fdd.FileName).ToUpper().Trim()) Isin = true;
                    }
                    if (Isin == true)
                    {
        ...
        private void DettechFile()
        {
            ...
            if (zipfile.GetFiles(false).Length > 0)
            {
                foreach (AbstractFile f in zipfile.GetFiles(false))
                {
                    if (Path.GetFileName(f.FullName.ToString()).ToUpper().Trim() == dfilename.ToUpper().Trim()) Isin = true;
            ...
            try
            {
                ...
                if (zipfile2.GetFiles(false).Length == 0) HasAttachments = false;
            }
            catch (Exception x1)
            {
                this.Cursor = System.Windows.Forms.Cursors.Default;
        ...
        private int CodexDocumentCaption(int ID, ref string DocCaption)
        {
            ...
            (FormMain as Form1).LockupTables.Tables["CodexPS_DAuthor"].PrimaryKey = new DataColumn[] { (FormMain as Form1).LockupTables.Tables["CodexPS_DAuthor"].Columns["A_ID"] };
            (FormMain as Form1).LockupTables.Tables["CodexPS_DType"].PrimaryKey = new DataColumn[] { (FormMain as Form1).LockupTables.Tables["CodexPS_DType"].Columns["T_ID"] };
            (FormMain as Form1).LockupTables.Tables["CodexPS_DStatus"].PrimaryKey = new DataColumn[] { (FormMain as Form1).LockupTables.Tables["CodexPS_DStatus"].Columns["C_ID"] };
            (FormMain as Form1).LockupTables.Tables["CodexPS_DCategory"].PrimaryKey = new DataColumn[] { (FormMain as Form1).LockupTables.Tables["CodexPS_DCategory"].Columns["C_ID"] };
            ...
            dr = (FormMain as Form1).LockupTables.Tables["CodexPS_DAuthor"].Rows.Find((int)dst.Tables[0].Rows[0]["C_Author"]);
            if (dr == null) Strauthor.Append(" "); else Strauthor.Append(@dr["A_Caption"].ToString().Trim());

            dr = (FormMain as Form1).LockupTables.Tables["CodexPS_DType"].Rows.Find((int)dst.Tables[0].Rows[0]["C_Type"]);
            if (dr == null) Strtype.Append(" "); else Strtype.Append(@dr["T_Caption"].ToString().Trim());

            // New ITEMS
            dr = (FormMain as Form1).LockupTables.Tables["CodexPS_DStatus"].Rows.Find((int)dst.Tables[0].Rows[0]["C_Status"]);
            ...
            dr = (FormMain as Form1).LockupTables.Tables["CodexPS_DCategory"].Rows.Find((int)dst.Tables[0].Rows[0]["C_Category"]);
        ...
        {
            string strsql = "Select C_ID FROM CodexPS_DDOCS WHERE C_ID = " + ID.ToString() + " ";
            SqlDataAdapter dacgl = new SqlDataAdapter(strsql, global::ILG.Codex.Codex2007.Properties.Settings.Default.ConnectionString);
            DataSet dst = new DataSet();
            dacgl.Fill(dst);
      ...
      //  private C1.C1Zip.C1ZipFile zfD;
        ...
        private void CreareLinks()
        {
            ...
            DLinks.WriteXml(ILG.Codex.Codex2007.Properties.Settings.Default.TemporaryDir + @"\Links.XML");
            DLinks.WriteXmlSchema(ILG.Codex.Codex2007.Properties.Settings.Default.TemporaryDir + @"\LinksSchema.XML");
        ...
        private void DOSaveDocument()
        {
            ...
            try
            {
                ...
                SqlCommand scoount = new SqlCommand("SELECT COUNT(C_ID) FROM CodexPS_DDocs");
                ...
                string StrInfo = "UPDATE CodexPS SET [C_Version] = 50,  [C_Date] = " + dtstr1 + ", [C_CodexPSDocs] = "+Quantity.ToString() +", " +
                             "[C_CodexDate] = " + dtstr1 + ",  [C_CodexPSVersion] = 65 WHERE [C_Version] = 50 ";
        ...
        private void DOEditDocument()
        {
            ...
            String UpdateCommand = "UPDATE [dbo].[CodexPS_DDOCS] SET [C_CAPTION] = @C_CAPTION, [C_AUTHOR] = @C_AUTHOR" +
        ...
        public void DODELDocument()
        {
            ...
            String DeleteCommand = "DELETE [dbo].[CodexPS_DDOCS] WHERE ([C_ID] = @Original_C_ID) ";
