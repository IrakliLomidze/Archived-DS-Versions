...
namespace ILG.Codex.CodexR4
{
    public partial class DocumentAddEdit : Form
    {
        ...
        public void NewDocumentMode()
        {
            ...
            #region Attachments
            if ((HasAttachments == true) && (DSRunTimeLicense.IsAttachmentShow() == true))
            {
        ...
        public int EditDocumentMode(int ID)
        {
            ...
            {
                if (DSRunTimeLicense.IsEnterInConfidentialDocumentAlowed() == false)
                {
           ...
           // CLinkGrid.InitializeVarialbles(2);
            ...
            if (DSRunTimeLicense.IsHistoryAccessGranted() == true)
            {
                ...
                if (DSRunTimeLicense.IsModifiedHistoryGranted() == true)
                {
                ...
                if (DSRunTimeLicense.IsDeletedInHistoryGranted() == true)
                {
        ...
        private void ultraToolbarsManager1_ToolClick(object sender, Infragistics.Win.UltraWinToolbars.ToolClickEventArgs e)
        {
            switch (e.Tool.Key)
            {
                ...
                case "DDDeleteDocument":    // ButtonTool
                    if (DSRunTimeLicense.IsDeleteAlowed() == true) DODELDocument();
                    break;
 ...
 //           CCategory.DisplayLayout.Grid.Width = CAuthor.Width;
      ...
      //  private C1.C1Zip.C1ZipFile zfD;
        ...
        private void DOSaveDocument()
        {
            ...
            if (DSRunTimeLicense.IsConfidentialSaveAllow() == false)
            {
    ...
    private void DOEditDocument()
        ...
        public void DODELDocument()
        {
            ...
            if (DSRunTimeLicense.IsDeleteAlowed() == false)
            {
        ...
        public bool HasDocumentHistory(int ID)
        {
            ...
            if (DSRunTimeLicense.IsHistoryExtendedAccessGranted() == true) strsql = $"Select Count(C_ID) from CodexDS_DDOCS_History WHERE C_ID = {ID.ToString()} ";
            bool result = false;
        ...
        private void CodexLoadHistoryItems(int ID)
        {
            ...
            if (DSRunTimeLicense.IsHistoryExtendedAccessGranted() == true)
                strsql = $"Select {Fields} from CodexDS_DDOCS_History " +
                            $" WHERE (C_ID = { ID.ToString() } ) AND (( H_STATUS = 0) OR (H_STATUS = 1)) Order By H_Date desc";

            if (DSRunTimeLicense.IsRecoverDeletedDocumentsGranted() == true)  // Only in Admin mode //IsDeletedInHistoryGranted() == true)
                strsql = $"Select {Fields} from CodexDS_DDOCS_History " +
