...
void CILISetupDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_BUTTON4, AcroButton);
	DDX_Control(pDX, IDC_STATIC12, Acro);
}
...
BOOL CILISetupDlg::OnInitDialog()
{
	...
	else
	{
		...
		// Check if XP Not SP2 Alet About That
		// Check if XP SXS Need Fix
		...
		{
			if (OSversion.wServicePackMajor < 3) 
			{
			   if (OSversion.wServicePackMajor < 2) 
			   {
				   MessageBox(_T("Your XP System Does not meet minimal system requirements \n Minimal System requrement for Codex is Windows XP with SP3 \n You may Continue Installation BUT WE DOES NOT GARANTED that System will work ok"));
				   CDialog::OnCancel();
				   return FALSE;
			   }
			
			   
			//	MessageBox(_T("Your XP System Does not have Installed Serive Pack 3 \n Minimal System requrement for Codex is Windows XP with SP3 \n You may Continue Installation BUT WE DOES NOT GARANTED that System will work ok"),_T(" "), MB_ICONHAND | MB_OK);
			//	if (MessageBox(_T("Do you want continue Installation? "),_T(" "),MB_YESNO | MB_ICONQUESTION | MB_DEFBUTTON2 ) == IDNO) { CDialog::OnCancel();return FALSE;}
				...
				// Install Older Version 1; Install SP3 0;
				if (w1.ResultX == 2) this->SQLExpress = 1;
				
				
				if (w1.ResultX == 1) 
				{
					// Install Hot Fix.
					// Before Check SXS.DLL
					ILI_SXS *iliz = new ILI_SXS();
					iliz->ILI_check();
					if (iliz->BuildVersion < 3295)
					{
							if (MessageBox(_T("To Install Codex.net 2007 on your system you need run sxs.dll hotfix \nWindowsXP-KB943232-v2-x86-ENU.exe  \n \n Would you like to install it? \n After installing hotfix system will reboot."),_T("System requriement"),
											MB_YESNO) == IDYES)	
								{
									// Install Windows Installer 3.1
									CString runcommand = theApp.CurrentPathStr;
									runcommand.Append(_T("\\wcu\\XPSP2\\HotFix\\WindowsXP-KB943232-v2-x86-ENU.exe "));
									//runcommand.Append(theApp.MSDECommandLine);
									DWORD d = ExecCmd2(runcommand);
									if (d != 0)
									{
										MessageBox(_T("Can't run Windows hot fix WindowsXP-KB943232-v2-x86-ENU.exe Installer Package"));
									}
									CDialog::OnCancel();
									return FALSE;
								}
...
{
  // Try to Install Adrobat 8
	CString runcommand = theApp.CurrentPathStr;
	runcommand.Append(_T("\\WCU\\Acrobat\\AdbeRdr812_en_US.exe"));
	DWORD d = ExecCmd2(runcommand.GetString());
