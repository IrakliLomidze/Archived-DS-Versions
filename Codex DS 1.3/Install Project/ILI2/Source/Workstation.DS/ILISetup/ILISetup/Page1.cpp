...
return (HBRUSH)(m_brush->GetSafeHandle());
	...
	if (OSversion.dwMajorVersion < 5)
	{
		...
		MessageBox(_T("Minimal System requirement is Windows XP SP3"));
	...
	{
		// Minimal System requrement in Windows XP SP2
		MessageBox(_T("Windows 2000 does not Supported By Codex 2007 DS \n Minimal System requirement in Windows XP SP2"));
		BI.EnableWindow(FALSE);	
		return;
	}
...
//		{
//			MessageBox(_T("Your XP System Does not have Service Pack 3 \n It is strongly Recommended to Install SP3 \n you may install Codex But now warranted that codex will work ok"));
//		}
	...
	if (ili12->ILI_checkMSDE() == 1)
	{
		...
		this->SetDlgItemText(IDC_SQLSTATIC,_T("You need to install SQL Server 2005 Express Edition 'Codex2007' Instance"));
		else
	    this->SetDlgItemText(IDC_SQLSTATIC,_T("You need to install SQL Server 2005 Express Edition OLDER EDITION 'Codex2007' Instance"));
	...
	{
		this->SetDlgItemText(IDC_SQLSTATIC,_T("SQL Server 2005 Express Edition With Advanced Service 'Codex2007' Instance"));
		CC_SQL = 0;
		m_sql2005 = false;
	}
	...
	{
		if (this->_SQLEXPRESS == 0)
		WaitLabelText = _T("Installing SQL Server 2005 Express, this may take a several time ...");
		else
		WaitLabelText = _T("Installing SQL Server 2005 Express (Windows XP SP2), this may take a several time ...");
		...
		if (SQLCOMBO.GetCurSel() == 1)
		{
		   // Without Advances Services
			// ======================================== 
			// SXS Check
			// if not SXS Compliant Exis With Error
			if (this->_SQLEXPRESS == 0)	
			{
					OSVERSIONINFOEX OSversion;
					ZeroMemory(&OSversion, sizeof(OSVERSIONINFOEX));
