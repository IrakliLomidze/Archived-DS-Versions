...
return (HBRUSH)(m_brush->GetSafeHandle());
	...
	case 0x0010 : { //Server Mode
		...
		case 0x0001 : { //Internal Mode
					...
					this->GetDlgItem(IDC_STATIC_SQL_SERVER_INSTALL)->ShowWindow(TRUE);
					this->GetDlgItem(IDC_STATIC_SQL_SERVER_INSTALL_SP)->ShowWindow(TRUE);
					this->GetDlgItem(IDC_STATIC_SQL_SERVER_INSTALL)->SetWindowText(_T("Install SQL Server 2005 Dev"));
					this->GetDlgItem(IDC_STATIC_SQL_SERVER_INSTALL_SP)->SetWindowText(_T("Install SQL Server SP 3"));
					if (SQLInstallMode == 1) {
						this->GetDlgItem(IDC_STATICMode)->ShowWindow(TRUE);
						this->l_InstallMode.SetWindowText(_T("Please Uninstall SQL Server Codex2007 Instance and Install it Again"));
						this->l_InstallMode.SetTextColor(RGB(255,0,0));
					}
				  } break;
			}
...
void CPage1::OnBnClickedStaticSqlServerInstall()
{
	...
	case 0x0010 : { //Server Mode
			...
			case 0x0001 : { //Internal Mode
					...
					if (IsWow64() == TRUE)
					{
						runcommand.Append(_T("\\Server\\SQL2005Dev\\X64\\Servers\\Setup.exe"));
					}
					else
					{
						runcommand.Append(_T("\\Server\\SQL2005Dev\\X86\\Servers\\Setup.exe"));
					}
					DWORD d = ExecCmd2(runcommand.GetString());
